...
def visualize_examples(test_loader):
    """
    Visualize the examples from the dataloader
    """
    import matplotlib.pyplot as plt

    examples = enumerate(test_loader)
    batch_idx, (example_data, example_targets) = next(examples)
    print("[INFO] example_data.shape {}".format(example_data.shape))
...
if __name__ == "__main__":
    ...
    ap.add_argument("-e", "--epochs", type=int, default= 2, help="# epochs to train for")
    ...
    if DEBUG: visualize_examples(dataloader)
    ...
    gen = Generator(inputDim= 100, outputDim = 512, outputChannels= 1, debug = False)
    gen.apply(weights_init)
    gen.to(DEVICE)

    # build the discriminator, initialize the weights, and flash it to the GPU/CPU
    print("[INFO] building discriminator...")
    disc = Discriminator(depth= 1, debug = False)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch + 1) % 2 == 0:
            ...
            p = os.path.join(file_path, "epoch_{}.png".format(
                str(epoch + 1).zfill(4)))
            cv2.imwrite(p, vis)
